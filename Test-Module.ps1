...
# .SYNOPSIS
#   cliHelper.core testScript v0.3.2
# .EXAMPLE
#   .\Test-Module.ps1 -version 0.3.2
#   Will test the module in ./BuildOutput/cliHelper.core/0.3.2/
# .EXAMPLE
#   .\Test-Module.ps1
#   Will test the latest module version in ./BuildOutput/cliHelper.core/
# .EXAMPLE
#   Import-Module .\cliHelper.core.psd1 -Verbose -Force; .\Test-Module.ps1 -NoBuild
...
param (
  ...
  [ValidateScript({
      ...
      $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.core')
...
begin {
  ...
  $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.core'))
  ...
  if ($BuildOutput.Exists) {
    ...
    $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.core', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
    if (![IO.Directory]::Exists("$BuildOutDir")) { throw [DirectoryNotFoundException]::New($BuildOutDir) }
  }
  $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir, "cliHelper.core.psd1"))
}

process {
  Write-Host "==========================================" -ForegroundColor Cyan
  Write-Host "  cliHelper.core Module - Test Suite" -ForegroundColor Cyan
  Write-Host "==========================================" -ForegroundColor Cyan
  ...
  if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "cliHelper.core.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [FileNotFoundException]::New("Module manifest file Was not Found in '$BuildOutDir'.") }
  $script:fnNames = [List[string]]::New(); $testFiles = [List[IO.FileInfo]]::New()
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.core.Integration.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.core.Features.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.core.Module.Tests.ps1')))
