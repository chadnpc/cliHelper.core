function New-ErrorRecord {
  ...
  param (
    ...
    [ArgumentCompleter({
        ...
        [ErrorManager]::AllExptnTypes.Typename | Where-Object { $_ -like "*$wordToComplete*" } | ForEach-Object { $CompletionResults.Add([System.Management.Automation.CompletionResult]::new($_, $_, "ParameterValue", $_)) }
  ...
  process {
    ...
    try {
      ...
      $ExcepnDescr = [string]$([ErrorManager]::AllExptnTypes | Where-Object { $_.TypeName -eq "$ExcepnTypeName" }).Description
    ...
    return $Execution.IsSuccess ? $Execution.Output : $Execution.Error
  }
}